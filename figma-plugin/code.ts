...
// ========================================
// FIGMA VARIABLES API
// Modern design tokens with Light/Dark mode support
// ========================================

async function generateVariables(): Promise<void> {
  figma.notify('Generating Figma Variables...');

  // ─────────────────────────────────────────────────────────────
  // 1. COLOR PRIMITIVES  (type: COLOR)
  //    All raw palette values — neutral, primary … pink
  // ─────────────────────────────────────────────────────────────
  ...
  const primMap: Record<string, Variable> = {};
  ...
  // ─────────────────────────────────────────────────────────────
  // 2. SEMANTIC COLORS  (type: COLOR, modes: Light / Dark)
  //    Aliases that point into Color Primitives
  // ─────────────────────────────────────────────────────────────
  ...
  for (const [scale, value] of Object.entries(tokens.spacing)) {
    ...
    v.setValueForMode(spaceModeId, value as number);
  }
  ...
  for (const [scale, value] of Object.entries(tokens.radii)) {
    ...
    v.setValueForMode(radModeId, value as number);
  }

  // ─────────────────────────────────────────────────────────────
  // 5. TYPOGRAPHY  (types: STRING + FLOAT)
  // ─────────────────────────────────────────────────────────────
  ...
  for (const [name, value] of Object.entries(tokens.typography.fontFamilies)) {
    ...
    v.setValueForMode(typModeId, value as string);
  }
  ...
  for (const [scale, value] of Object.entries(tokens.typography.fontSizes)) {
    ...
    v.scopes = ['FONT_SIZE'];
    v.setValueForMode(typModeId, value as number);
  }

  // 5c. Font weights  →  FLOAT  (all 9: thin → black)
  ...
  // ─────────────────────────────────────────────────────────────
  // 6. COMPONENT TOKENS  (type: FLOAT)
  //    Component-level sizing/spacing/radius values
  // ─────────────────────────────────────────────────────────────
  ...
  // ─────────────────────────────────────────────────────────────
  // 7. ANIMATION TOKENS  (types: FLOAT + STRING)
  //    Durations in ms (FLOAT) and easing curves (STRING)
  //    Note: Figma has no built-in scope for these yet, so they
  //    use ALL_SCOPES and serve as documentation/reference values.
  // ─────────────────────────────────────────────────────────────
  ...
  // 7a. Durations  →  FLOAT (milliseconds)
  for (const [name, ms] of Object.entries(tokens.animations.durations)) {
    ...
    v.scopes = ['ALL_SCOPES'];
    v.setValueForMode(animModeId, ms as number);
  }

  // 7b. Easing functions  →  STRING (cubic-bezier / linear)
...
async function generateToggle(): Promise<void> {
  await loadFonts();
