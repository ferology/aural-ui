...
- **Maximum selections:**
  ...
  argTypes: {
    options: {
      control: 'object',
      description:
        'Array of option objects with value and label properties ({ value: string, label: string, group?: string }), can include optional group for categorization',
    },
    selected: {
      control: 'object',
      description:
        'Array of currently selected option values (e.g., ["option1", "option2"]), displayed as removable chips in the trigger',
    },
    placeholder: {
      control: 'text',
      description:
        'Placeholder text displayed in the trigger when no items are selected (e.g., "Select skills..." or "Choose team members...")',
    },
    disabled: {
      control: 'boolean',
      description:
        'Disabled state prevents all interaction, applies muted styling with 50% opacity, and disables chip remove buttons',
    },
    searchable: {
      control: 'boolean',
      description:
        'Enable real-time search/filter functionality with search input at top of dropdown for finding options by label (case-insensitive)',
    },
    maxHeight: {
      control: 'text',
      description:
        'Maximum height of dropdown options list in CSS units (e.g., "300px", "20rem"), applies scrollbar when exceeded',
    },
    size: {
      control: 'select',
      options: ['sm', 'md', 'lg'],
